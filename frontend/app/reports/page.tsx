  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-500 text-sm mt-1">Generate GSTR-1 and GSTR-3B simulation reports</p>
      </div>
        ...
        <div className="space-y-4">
          ...
          {reportType === "GSTR-1" && data.summary && (
            <>
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="card p-4"><p className="text-xs text-slate-500">CGST</p><p className="text-lg font-bold text-slate-800 mt-1">{fmt(data.summary.total_cgst)}</p></div>
                <div className="card p-4"><p className="text-xs text-slate-500">SGST</p><p className="text-lg font-bold text-slate-800 mt-1">{fmt(data.summary.total_sgst)}</p></div>
                <div className="card p-4"><p className="text-xs text-slate-500">IGST</p><p className="text-lg font-bold text-slate-800 mt-1">{fmt(data.summary.total_igst)}</p></div>
              </div>
            </>
          )}
          ...
          {reportType === "GSTR-3B" && data["6_payment_of_tax"] && (
            <div className="grid grid-cols-3 gap-4">
              {[
                ...
                <div key={c.label} className={`card p-5 ${c.highlight ? "bg-red-50 border-red-200" : ""}`}>
                  <p className="text-xs text-slate-500 uppercase tracking-wide">{c.label}</p>
          ...
          <div className="card p-5">
            ...
            <pre className="bg-slate-900 text-slate-300 rounded-lg p-4 text-xs overflow-auto max-h-64 font-mono">
